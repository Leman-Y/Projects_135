...
bool isAlpha(char c) // checks for characters that are not alphabetical
{
	return ((c >= 'a' && c <= 'z') || (c >= 'A' && c <= 'Z')
	        || (c == static_cast<char>(39))); // apostrophe(39) allowed
}
//////////////////////////////////////////////////////////////////////////////////////////////
...
bool stringIsAlpha(string inputword) // checks to see if a given string is alphabetical
{
	for (int i = 0; i < inputword.size(); i++) {
		if (!(isAlpha(inputword[i]))) // if even ONE letter is not alphatical then the entire word
		                              // is nonalphabetical
		{
			return false;
		}
...
bool is_In_Dict(string inputword) // checks to see if the word can be found in the dictionary
{
	...
	while (getline(cmu, str)) // using getline to in order to stop reading when it reaches newline
	{
		...
		if (inputword == word && stringIsAlpha(inputword)) // if the inputed word is found in the
		                                                   // dictionary AND is alphabetical...
		{
...
/////////////////////////////////////////////////////////////////////////////////
void splitOnSpace(string s, string& before, string& after) { // splits words on space
	// reset strings
	before = "";
	after = "";
...
int numOfPhonemes(string pronunciation) // finds the number of phonemes in a pronunciation
{
	...
	for (int i = 0; i < pronunciation.size(); i++) {
		if (isspace(pronunciation[i - 1])) // every time there is a space before a character, it
...
//////////////////////////////////////////////////////////////////////////////////////////
bool is_replace_phoneme(string s1,
                        string s2) // this function checks whether the first pronunciation
// is the same as the second one if one phoneme is replaced
{
	...
	if (diffpho == 1) // if only one change occured,that means the pronunciation will be the same if
	                  // one phoneme is replaced
	{
		return true;
	}
	return false;
...
bool is_add_phoneme(string s1, string s2) {
	...
	int samepho = 0;
	for (int i = 0; i < phonemecount1; i++) {
		if (getNthPhoneme(s1, i) == getNthPhoneme(s2, i)
		    || getNthPhoneme(s1, i) == getNthPhoneme(s2, i + 1)) {
			samepho++; // increment for every identical phoneme
		}
	}
	if (samepho == numOfPhonemes(s1)) // if the word being compared has the identical phonemes of
	                                  // the original
...
bool is_remove_phoneme(string s1, string s2) // opposite of is_add_phoneme s1 and s2 switched
{
	...
	for (int i = 0; i < phonemecount2; i++) {
		if (getNthPhoneme(s2, i) == getNthPhoneme(s1, i)
		    || getNthPhoneme(s2, i) == getNthPhoneme(s1, i + 1)) {
			samepho++;
		}
	}
...
//   {
//     emptys1+=getNthPhoneme(s1,i);//fill up the first empty string with the original without
//     spaces
//   }
//   for (int k=0;k<s2count;k++)
//   {
//     if (getNthPhoneme(s1,k)==getNthPhoneme(s2,k))//If the phoneme is the same add it to the empty
//     string
//      {
//        emptys2+=getNthPhoneme(s2,k);
//      }
//     else if ((k+1)<s1count && getNthPhoneme(s1,k)!=getNthPhoneme(s2,k))//if they are not then add
//     k+1 phoheme
//     {
...
//
//   }
//   if (emptys1==emptys2)//if the second string ends up being equal to the original, then that
//   means only one phoneme needed to be removed
//   {
//     return true;
//   }
//   return false;
